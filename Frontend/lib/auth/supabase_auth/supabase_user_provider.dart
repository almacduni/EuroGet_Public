...
class EurogetArchiveSupabaseUser extends BaseAuthUser {
  EurogetArchiveSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> eurogetArchiveSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EurogetArchiveSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
